<div class="br-mainpanel mg-0">
    <div class="br-pagebody pd-0 mg-0">
        ...
        <br>
        <br>
        <a href="/public/guide/CompanyEncode-Import-Template.xlsx" class="tx-bold tx-orange" target="_blank" title="Download Template" download="Import-CompanyEncode-Template.xlsx"><i class="fa fa-file-excel-o"></i> DOWNLOAD TEMPLATE</a>
        <p class="msg mg-t-30 tx-danger hidden"></p>
        <div class="list hidden">
            <table class="table table-bordered bd mg-t-30">
                <thead class="bg-gray-100">
                    <th><center><input name="check_all_import" id="check_all_import" type="checkbox" checked></center></th>
                    <th>NAME</th>
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.upload', function(e){
            ...
            $.ajax({
                url: '/company/import-bulk-encode-json/',
                type: 'POST',
                data: fd,
                processData: false,
                contentType: false,
                    ...
                    if(typeof data.row != "undefined" && data.row != null && data.row.length > 0){
                        ...
                        $.each(data.row, function(index, item) {
                            ...
                            list += '<tr class="'+disabled+'">'+
                                        ...
                                        '<td>'+(item['company_name'] == null ? '' : item['company_name'])+'</td>'+
                                        '<td>'+(item[''] == null ? '' : item[''].replace(/[,]+/g, '<br>'))+'</td>'+
                                        '<td>'+(item['contact_no'] == null ? '' : item['contact_no'])+'</td>'+
